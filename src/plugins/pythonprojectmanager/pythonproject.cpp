...
#include "fileformat/pythonprojectitem.h"
        ...
        if (!m_projectItem) {
            ...
            if (reader.fetch(m_fileName)) {
                ...
                if (component->isReady()
                    ...
                    connect(m_projectItem.data(), SIGNAL(pythonFilesChanged(QSet<QString>, QSet<QString>)),
...
ProjectExplorer::IProjectManager *PythonProject::projectManager() const
{
    return m_manager;
}
...
ProjectExplorer::ProjectNode *PythonProject::rootProjectNode() const
{
    return m_rootNode;
}
...
bool PythonProject::fromMap(const QVariantMap &map)
{
    ...
    if (targets().isEmpty()) {
        ...
        Internal::PythonProjectTarget *target = factory->create(this, QLatin1String(Constants::PYTHON_VIEWER_TARGET_ID));
        addTarget(target);
    }
